...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || ( get_site_option('active_sitewide_plugins') && array_key_exists( 'woocommerce/woocommerce.php', get_site_option('active_sitewide_plugins') ) ) ) {
	...
	add_shortcode('CUSTOM_ADD_TO_CART', 'qib_quantity_field_archive');
	...
	function qib_quantity_field_archive() {		
		...
		if ( $product->is_purchasable() && ! $product->is_sold_individually() && $product->is_in_stock() && 'variable' != $product->get_type() && 'bundle' != $product->get_type() ) {
			woocommerce_quantity_input( [ 'min_value' => 1, 'max_value' => $product->backorders_allowed() ? '' : $product->get_stock_quantity(),'product_id'=>$product->get_id() ,'product_name'=>$product->get_title()] );
		}
	}	
	...
	function add_to_cart() {
		...
		if ( $cart_item['product_id'] == $_POST['id'] ) {
		...
		if($product_key){
			...
			$quantity=1;
			WC()->cart->add_to_cart( $_POST['id'], $quantity);
		}
		}
	...
	add_action('wp_ajax_nopriv_arabinda_add_to_cart', 'add_to_cart');
	add_action('wp_ajax_arabinda_add_to_cart', 'add_to_cart');

function register_plugin_styles() {
    if(wp_doing_ajax()){
        return;
	}
	wp_enqueue_script( 'add_to_cart', plugin_dir_url( __FILE__ ).'js/ArabindaCool.min.js', array('jquery'));
    wp_localize_script( 'add_to_cart', 'ArabindaCool', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
}
